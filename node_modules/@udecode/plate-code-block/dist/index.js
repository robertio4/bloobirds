...
const indentCodeLine = (editor, {
  ...
  if (!plateCore.isExpanded(editor.selection)) {
    ...
    if (/\S/.test(text)) {
      plateCore.insertText(editor, indent, {
        at: editor.selection
      });
      return;
    }
...
const normalizeCodeBlock = editor => {
  ...
  return ([node, path]) => {
    ...
    if (isCodeBlockRoot) {
      ...
      const nonCodeLine = plateCore.getChildren([node, path]).find(([child]) => child.type !== codeLineType);
...
const isCodeBlockEmpty = editor => {
  ...
  const codeLines = Array.from(plateCore.getChildren(codeBlock));
  if (codeLines.length === 0) return true;
  if (codeLines.length > 1) return false;
  const firstCodeLineNode = codeLines[0][0];
  return !plateCore.getNodeString(firstCodeLineNode);
};
...
const isSelectionAtCodeBlockStart = editor => {
  ...
  if (!selection || plateCore.isExpanded(selection)) return false;
  const {
    codeBlock
  } = (_getCodeLineEntry = getCodeLineEntry(editor)) !== null && _getCodeLineEntry !== void 0 ? _getCodeLineEntry : {};
  if (!codeBlock) return false;
  return plateCore.isStartPoint(editor, selection.anchor, codeBlock[1]);
};
...
const unwrapCodeBlock = editor => {
  ...
  plateCore.withoutNormalizing(editor, () => {
    const codeBlockEntries = plateCore.getNodeEntries(editor, {
      at: editor.selection,
      match: {
        type: codeBlockType
      }
    ...
    for (const codeBlockEntry of reversedCodeBlockEntries) {
      const codeLineEntries = plateCore.getChildren(codeBlockEntry);
