...
const onKeyDownAutoformat = (editor, {
  ...
  const before = plateCore.getPointBefore(editor, end, {
    unit: 'character',
    distance: 1
  });
  ...
  const char = plateCore.getEditorString(editor, charRange);
  ...
  plateCore.deleteBackward(editor, {
    unit: 'character'
  ...
  if (rule && typeof rule.match === 'string') {
    plateCore.insertText(editor, rule.match);
  } else {
    const matchArray = rule.match;

    if (matchArray && matchArray.length > 0) {
      plateCore.insertText(editor, matchArray[0]);
    }
  }
...
const autoformatBlock = (editor, {
  ...
  for (const match of matches) {
    ...
    if (triggerAtBlockStart) {
      matchRange = plateCore.getRangeFromBlockStart(editor); // Don't autoformat if there is void nodes.

      const hasVoidNode = plateCore.someNode(editor, {
        at: matchRange,
        match: n => plateCore.isVoid(editor, n)
      });
      if (hasVoidNode) continue;
      const textFromBlockStart = plateCore.getEditorString(editor, matchRange);
      if (end !== textFromBlockStart) continue;
    } else {
      matchRange = plateCore.getRangeBefore(editor, editor.selection, {
        matchString: end
      });
    ...
    if (!allowSameTypeAbove) {
      ...
      const isBelowSameBlockType = plateCore.someNode(editor, {
        match: {
          type
        }
      });
    ...
    if (!format) {
      plateCore.setElements(editor, {
        ...
        match: n => plateCore.isBlock(editor, n)
      });
    } else {
      format(editor);
    }
...
const isPreviousCharacterEmpty = (editor, at) => {
  const range = plateCore.getRangeBefore(editor, at);
...
const getMatchPoints = (editor, {
  ...
  if (end) {
    beforeEndMatchPoint = plateCore.getPointBeforeLocation(editor, selection, {
      matchString: end
    });
  ...
  if (start) {
    afterStartMatchPoint = plateCore.getPointBeforeLocation(editor, beforeEndMatchPoint, {
      matchString: start,
      skipInvalid: true,
      afterMatch: true
    });
    if (!afterStartMatchPoint) return;
    beforeStartMatchPoint = plateCore.getPointBeforeLocation(editor, beforeEndMatchPoint, {
      matchString: start,
      skipInvalid: true
    });
...
const autoformatMark = (editor, {
  ...
  for (const match of matches) {
    ...
    if (!ignoreTrim) {
      const matchText = plateCore.getEditorString(editor, matchRange);
    ...
    plateCore.select(editor, matchRange);
    marks.forEach(mark => {
  ...
  for (const match of matches) {
    ...
    if (typeof format === 'function') {
      ...
      if (beforeStartMatchPoint) {
        ...
        plateCore.insertText(editor, formatStart, {
...
const withAutoformat = (editor, {
  ...
  editor.insertText = text => {
    if (!plateCore.isCollapsed(editor.selection)) return insertText(text);
...
const createAutoformatPlugin = plateCore.createPluginFactory({
  key: KEY_AUTOFORMAT,
