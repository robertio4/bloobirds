  ...
  if (plateCore.isCollapsed(editor.selection)) {
    rules.forEach(({
      ...
      if (hotkey && isHotkey(hotkey, event)) {
        if (predicate(editor) && plateCore.someNode(editor, {
          match: {
            type: types
          }
        })) {
...
const createResetNodePlugin = plateCore.createPluginFactory({
  key: KEY_RESET_NODE,
  handlers: {
  ...
  withOverrides: (editor, {
    ...
    if (!options.disableEditorReset) {
      const deleteFragmentPlugin = () => {
        ...
        if (!selection) return;
        const start = plateCore.getStartPoint(editor, []);
        const end = plateCore.getEndPoint(editor, []);
        ...
        if (slate.Point.equals(selection.anchor, start) && slate.Point.equals(selection.focus, end) || slate.Point.equals(selection.focus, start) && slate.Point.equals(selection.anchor, end)) {
          plateCore.resetEditorChildren(editor, {
            insertOptions: {
              select: true
            }
          });
      ...
      editor.deleteBackward = unit => {
        ...
        if (selection && plateCore.isCollapsed(selection)) {
          ...
          if (slate.Point.equals(selection.anchor, start)) {
            const node = plateCore.getNode(editor, [0]);
            const {
              children,
              ...props
            } = editor.blockFactory({}, [0]); // replace props

            plateCore.withoutNormalizing(editor, () => {
              plateCore.unsetNodes(editor, Object.keys(plateCore.getNodeProps(node)), {
                at: [0]
              });
              plateCore.setNodes(editor, props, {
                at: [0]
              });
            });
            return;
