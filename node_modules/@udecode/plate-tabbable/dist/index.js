...
const findTabDestination = (editor, {
  ...
  if (activeTabbableEntry) {
    ...
    if (direction === 'forward') {
      const pointAfter = plateCore.getPointAfter(editor, activeTabbableEntry.path);
    ...
    return {
      type: 'path',
      path: plateCore.getPoint(editor, activeTabbableEntry.path).path
    };
  }
...
const TabbableEffects = () => {
  ...
  const {
    ...
    isTabbable
  } = plateCore.getPluginOptions(editor, KEY_TABBABLE);
  react.useEffect(() => {
    ...
    const handler = event => {
      ...
      const defaultTabbableEntries = tabbableDOMNodes.map(domNode => {
        const slateNode = plateCore.toSlateNode(editor, domNode);
        if (!slateNode) return;
        return {
          domNode,
          slateNode,
          path: plateCore.findNodePath(editor, slateNode)
        };
...
const createTabbablePlugin = plateCore.createPluginFactory({
  key: KEY_TABBABLE,
  ...
  options: {
    ...
    insertTabbableEntries: () => [],
    isTabbable: (editor, tabbableEntry) => plateCore.isVoid(editor, tabbableEntry.slateNode)
  }
});
