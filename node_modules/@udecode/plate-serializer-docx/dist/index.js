...
const cleanDocxBrComments = rootNode => {
  plateCore.traverseHtmlElements(rootNode, element => {
    if (element.tagName !== 'BR') {
      return true;
    }

    if (element.nextSibling && plateCore.isHtmlComment(element.nextSibling) && element.nextSibling.data === '[if !supportLineBreakNewLine]') {
      plateCore.removeHtmlNodesBetweenComments(element.nextSibling, '[if !supportLineBreakNewLine]', '[endif]');
    }
...
const isHtmlOpEmpty = element => element.nodeName === 'O:P' && element.textContent === plateCore.NO_BREAK_SPACE;
...
const getRtfImageHex = imageData => {
  ...
  if (bracketSplit.length > 2 && beforeBracket.includes('blipuid')) {
    return afterBracket.split(plateCore.SPACE).join('');
  }

  const spaceSplit = beforeBracket.split(plateCore.SPACE);
...
const END_OF_ID_SEQUENCE_CHARACTERS = ['\\', '{', plateCore.CARRIAGE_RETURN, plateCore.LINE_FEED, plateCore.SPACE];
...
const getVShapes = document => {
  const comments = plateCore.getHtmlComments(document);
...
const cleanDocxImageElements = (document, rtf, rootNode) => {
  ...
  plateCore.traverseHtmlElements(rootNode, element => {
    ...
    const dataUri = `data:${rtfImage.mimeType};base64,${plateCore.hexToBase64(rtfImage.hex)}`;
...
const getDocxListContentHtml = rootElement => {
  ...
  plateCore.removeHtmlNodesBetweenComments(clonedElement, '[if !supportLists]', '[endif]');
  plateCore.traverseHtmlElements(clonedElement, element => {
    const styleAttribute = element.getAttribute('style');
...
const isDocxOl = element => {
  ...
  return plateCore.isOlSymbol(listNode.textContent || '');
};
...
const docxListToList = element => {
  ...
  const list = plateCore.parseHtmlElement(`<${listTagName}>${listHtml}</${listTagName}>`);
  return {
    list,
    nextSibling
  };
...
const cleanDocxQuotes = rootNode => {
  plateCore.traverseHtmlElements(rootNode, element => {
    if (element.parentNode && element.tagName === 'P' && element.classList.contains('MsoQuote')) {
      plateCore.replaceTagName(element, 'blockquote');
    }
...
const isDocxContent = body => {
  let result = false;
  plateCore.traverseHtmlElements(body, element => {
    const styleAttribute = element.getAttribute('style') || '';
...
const cleanDocx = (html, rtf) => {
  const document = new DOMParser().parseFromString(plateCore.preCleanHtml(html), 'text/html');
  const {
    body
  } = document;
  ...
  plateCore.cleanHtmlEmptyElements(body);
  cleanDocxEmptyParagraphs(body);
  cleanDocxQuotes(body);
  cleanDocxSpans(body);
  plateCore.cleanHtmlTextNodes(body);
  cleanDocxBrComments(body);
  plateCore.cleanHtmlBrElements(body);
  plateCore.cleanHtmlLinkElements(body);
  plateCore.cleanHtmlFontElements(body);
  cleanDocxListElements(body);
  plateCore.copyBlockMarksToSpanChild(body);
  return plateCore.postCleanHtml(body.innerHTML);
};
...
const createDeserializeDocxPlugin = plateCore.createPluginFactory({
