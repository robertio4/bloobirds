    ...
    if (plateCore.isElement(node)) {
      ...
      if (getCellTypes(editor).includes(node.type)) {
        ...
        if (plateCore.isText(children[0])) {
          plateCore.wrapNodeChildren(editor, editor.blockFactory({}, path), {
            at: path
          });
          return;
        }
  ...
  editor.setFragmentData = (data, originEvent) => {
    ...
    plateCore.withoutNormalizing(editor, () => {
      tableRows.forEach((row, rowIndex) => {
        ...
        const cellStrings = [];
        const rowElement = row.type === plateCore.getPluginType(editor, ELEMENT_TH) ? document.createElement('th') : document.createElement('tr');
        rowCells.forEach((_, cellIndex) => {
          const cellPath = rowPath.concat(x + cellIndex); // select cell by cell

          plateCore.select(editor, {
            anchor: plateCore.getStartPoint(editor, cellPath),
            focus: plateCore.getEndPoint(editor, cellPath)
      ...
      plateCore.select(editor, initialSelection);
