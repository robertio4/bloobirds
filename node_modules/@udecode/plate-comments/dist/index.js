...
const CommentsProvider = ({
  ...
  return /*#__PURE__*/React__default["default"].createElement(plateCore.JotaiProvider, {
    initialValues: plateCore.getJotaiProviderInitialValues(commentsStore, props),
    scope: SCOPE_COMMENTS
  }, children);
};
...
const useNewCommentText = () => {
  ...
  return plateCore.getNodeString(editingValue === null || editingValue === void 0 ? void 0 : editingValue[0]);
};
...
const useAddComment = () => {
  ...
  return value => {
    ...
    if (!myUserId) return;
    const id = (_value$id = value.id) !== null && _value$id !== void 0 ? _value$id : plateCore.nanoid();
    setComments({ ...comments,
      [id]: {
        id,
        userId: myUserId,
...
const CommentProvider = ({
  ...
  return /*#__PURE__*/React__default["default"].createElement(plateCore.JotaiProvider, {
    initialValues: plateCore.getJotaiProviderInitialValues(commentStore, props),
...
const useCommentText = scope => {
  ...
  if (!comment) return null;
  return plateCore.getNodeString((_comment$value = comment.value) === null || _comment$value === void 0 ? void 0 : _comment$value[0]);
};
...
const findCommentNode = (editor, options) => {
  return plateCore.findNode(editor, {
...
const unsetCommentNodesById = (editor, {
  ...
  plateCore.unsetNodes(editor, getCommentKey(id), {
    at: [],
...
const getCommentNodeEntries = editor => {
  return [...plateCore.getNodeEntries(editor, {
    at: [],
...
const getCommentNodesById = (editor, id) => {
  return Array.from(plateCore.getNodeEntries(editor, {
    at: [],
    match: n => isCommentNodeById(n, id)
  }));
};
...
const getCommentPosition = (editor, node) => {
  const DOMNode = plateCore.toDOMNode(editor, node);
  if (!DOMNode) return;
  const DOMNodePosition = getElementAbsolutePosition(DOMNode);
  const editorDOMNode = plateCore.toDOMNode(editor, editor);
...
const useAddCommentMark = () => {
  ...
  return () => {
    ...
    if (!plateCore.isExpanded(selection)) return;
    const id = plateCore.nanoid(); // add comment prop to inline elements
...
const useHooksComments = (editor, {
  ...
  plateCore.useHotkeys(hotkey, e => {
    if (!editor.selection) return;
    e.preventDefault(); // block comments

    if (!plateCore.isExpanded(editor.selection)) return;
    addCommentMark();
  ...
  editor.normalizeNode = entry => {
    ...
    if (node[MARK_COMMENT]) {
      if (getCommentCount(node) < 1) {
        plateCore.unsetNodes(editor, MARK_COMMENT, {
          at: path
        });
        return;
      }
...
const createCommentsPlugin = plateCore.createPluginFactory({
  key: MARK_COMMENT,
  isLeaf: true,
...
const AvatarImage = plateCore.createComponentAs(props => {
...
const useCommentEditTextarea = ({
  ...
  const ref = plateCore.useComposedRef(textareaRef, _ref);
  React.useEffect(() => {
    setTimeout(() => {
...
const CommentEditTextarea = plateCore.createComponentAs(props => {
  const htmlProps = useCommentEditTextarea(props);
  return plateCore.createElementAs('textarea', htmlProps);
});
...
const CommentLinkButton = plateCore.createComponentAs(props => {
...
const CommentLinkDialogCloseButton = plateCore.createComponentAs(props => {
...
const CommentLinkDialogCopyLink = plateCore.createComponentAs(props => {
...
const CommentLinkDialogInput = plateCore.createComponentAs(props => {
...
const CommentNewTextarea = plateCore.createComponentAs(props => {
...
const CommentUserName = plateCore.createComponentAs(props => {
...
const CommentsPositioner = plateCore.createComponentAs(props => {
