...
const exitBreakAtEdges = (editor, {
  ...
  if (start || end) {
    ...
    if (start && plateCore.isSelectionAtBlockStart(editor)) {
      isEdge = true;
      isStart = true;
    }

    if (end && plateCore.isSelectionAtBlockEnd(editor)) {
      isEdge = true;
    }

    if (isEdge && plateCore.isExpanded(editor.selection)) {
...
const exitBreak = (editor, {
  ...
  if (before) {
    ...
    insertPath = slate.Path.next(slicedPath);
  }
...
const onKeyDownExitBreak = (editor, {
  ...
  rules.forEach(({
    ...
    if (isHotkey(hotkey, event) && plateCore.queryNode(entry, rule.query)) {
...
const createExitBreakPlugin = plateCore.createPluginFactory({
  key: KEY_EXIT_BREAK,
  handlers: {
...
const onKeyDownSingleLine = () => event => {
  if (plateCore.Hotkeys.isSplitBlock(event)) {
    event.preventDefault();
  }
};
...
const createSingleLinePlugin = plateCore.createPluginFactory({
  key: KEY_SINGLE_LINE,
  handlers: {
...
const onKeyDownSoftBreak = (editor, {
  ...
  rules.forEach(({
    ...
    if (isHotkey(hotkey, event) && plateCore.queryNode(entry, query)) {
      event.preventDefault();
      event.stopPropagation();
      editor.insertText('\n');
    }
