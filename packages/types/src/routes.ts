import { BobjectId, BobjectTypes } from './bobjects';
...
export const cadenceEditUrl = cadenceId =>
  `${APP_PLAYBOOK_CADENCES_EDIT}?cadence=${encodeURIComponent(cadenceId)}`;

export const cadenceEditUrlV2 = cadenceId =>
  `${APP_CADENCES_EDIT}?cadence=${encodeURIComponent(cadenceId)}`;
...
export const bobjectUrl = (
  bobject: { id: Partial<BobjectId> },
  referencedBobject = { id: undefined },
) => {
  if (!bobject) {
    return '';
  }
