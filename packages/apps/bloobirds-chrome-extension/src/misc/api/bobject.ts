...
import {
  ...
  relativeDates,
  transformDate,
} from '../../content/components/filter/RelativeDates';
...
const buildUrl = (accountId, bobjectType, bobjectId?) => {
...
const typeRequestBuilder = (accountId, bobjectType) => ({
  get: bobjectId =>
    perform({ url: `${buildUrl(accountId, bobjectType, bobjectId)}/raw`, method: 'GET' }),
  search: searchRequest =>
    perform({
      url: `${buildUrl(accountId, bobjectType)}/search${
  ...
  downloadSearch: searchRequest =>
    perform({
      url: `${buildUrl(accountId, bobjectType)}/search/download`,
      method: 'POST',
      body: preProcessSearchRequest(searchRequest),
    }),
  delete: bobjectId =>
    perform({ url: buildUrl(accountId, bobjectType, bobjectId), method: 'DELETE' }),
  getForm: (bobjectId, queryParams) => {
    let url = `${buildUrl(accountId, bobjectType, bobjectId)}/form`;
  ...
  create: data => perform({ url: buildUrl(accountId, bobjectType), method: 'POST', body: data }),
  bulkSet: bobjects =>
    perform({ url: `${buildUrl(accountId, bobjectType)}/bulk`, method: 'POST', body: bobjects }),
  bulkPartialSet: bobjects =>
    perform({ url: `${buildUrl(accountId, bobjectType)}/bulk`, method: 'PATCH', body: bobjects }),
  bulkPutSet: bobjects =>
    perform({ url: `${buildUrl(accountId, bobjectType)}/bulk`, method: 'PUT', body: bobjects }),
  bulkDelete: bobjects =>
    perform({
      url: `${buildUrl(accountId, bobjectType)}/delete/bulk`,
      method: 'POST',
      body: bobjects,
    }),
  set: ({ bobjectId, fields }) =>
    perform({
      url: `${buildUrl(accountId, bobjectType, bobjectId)}/raw`,
      method: 'POST',
      body: fields,
    }),
  ...
  aggregation: aggregationRequest =>
    perform({
      url: `${buildUrl(accountId, bobjectType)}/aggregation`,
      method: 'POST',
...
const BobjectApi = {
  request: accountId => ({
    bobjectType: bobjectType => typeRequestBuilder(accountId, bobjectType),
    Activity: () => typeRequestBuilder(accountId, ACTIVITY_BOBJECT_TYPE_NAME),
    Task: () => typeRequestBuilder(accountId, TASK_BOBJECT_TYPE_NAME),
    Company: () => typeRequestBuilder(accountId, COMPANY_BOBJECT_TYPE_NAME),
    Lead: () => typeRequestBuilder(accountId, LEAD_BOBJECT_TYPE_NAME),
    Opportunity: () => typeRequestBuilder(accountId, OPPORTUNITY_BOBJECT_TYPE_NAME),
  }),
};
