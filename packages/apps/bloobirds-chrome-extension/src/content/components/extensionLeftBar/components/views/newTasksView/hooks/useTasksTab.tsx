...
import { BobjectTypes, Filter } from '@bloobirds-it/types';
...
import { ClusteredTaskList, TaskFeedRequest, TaskFeedTaskList } from '../types';
...
function useTaskFeedContext() {
  ...
  const tasks = useTasks({
    filterValues: useTaskFeedFilterValues(),
...
function TaskFeedProvider({ children }) {
...
export const useTasksGlobalAggregation = () => {
  ...
  const { data, mutate } = useSWR(
    '/left-bar/aggregation',
    () => api.post(`/bobjects/${accountId}/task/aggregation`, {}),
...
function useTasks({ filterValues, pagination, sort, configuration }) {
  const accountId = useActiveAccountId();
  ...
  const { data, isLoading, mutate } = useSWR<ClusteredTaskList>(
    'task-feed' + JSON.stringify(taskFeedRequest),
    () => api.post(`/bobjects/${accountId}/task/feed`, taskFeedRequest).then(res => res.data),
    {
