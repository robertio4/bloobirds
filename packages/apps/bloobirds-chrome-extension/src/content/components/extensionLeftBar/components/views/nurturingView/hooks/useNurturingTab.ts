...
import { useCustomTasks, useDidMountEffect, useNoStatusOppSetting } from '@bloobirds-it/hooks';
import { useAggregationSubscription } from '@bloobirds-it/plover';
import {COMPANY_FIELDS_LOGIC_ROLE,
...
import { useSalesforceStatusFilter } from '../../pipelineView/hooks/useSalesforceStatusFilter';
import { BASE_SEARCH_REQUEST, Stages } from '../../view.utils';
import { TASK_COLUMNS, TASK_REFERENCED_COLUMNS } from '../nurturingTab.constants';
...
const getQueries = (
  stage: Stages,
  statuses: { id: string }[],
  isNoStatusOppSetting: boolean,
  oppNoStatusQuery: { [x: string]: string[] | { query: any[]; searchMode: string } },
) => {
  const queries = [
    {
    ...
    },
    {
      ...
      [TASK_FIELDS_LOGIC_ROLE.OPPORTUNITY]: isNoStatusOppSetting
...
export const fetchTasks = (
  ...
  oppNoStatusQuery: Record<string, string[]>,
...
export const useNurturingGlobalAggregation = (key: string, taskTypes: any) => {
  const { useGetDataModel, useGetSettings } = useExtensionContext();
  const dataModel = useGetDataModel();
  const settings = useGetSettings();
