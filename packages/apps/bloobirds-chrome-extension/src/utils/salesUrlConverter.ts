import { isLinkedInProfilePage, normalizeUrl } from './url';
...
export async function getLinkedInUrlFromSalesProfile(profileUrl: string): Promise<string> {
  const cache: string = localStorage.getItem(cacheKey);
  const cacheJson: any = cache ? JSON.parse(cache) : {};
  ...
  await timeout(1500);
  const copyButton: HTMLElement = document.querySelector('div[data-control-name="copy_linkedin"]');
