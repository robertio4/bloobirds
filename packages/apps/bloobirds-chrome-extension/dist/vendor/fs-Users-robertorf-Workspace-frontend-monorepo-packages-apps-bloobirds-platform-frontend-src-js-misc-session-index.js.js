import RoleManager from '/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-apps-bloobirds-platform-frontend-src-js-misc-session-RoleManager.js.js';
...
const b64DecodeUnicode = str => {
  ...
  return decodeURIComponent(
    Array.prototype.map
      .call(atob(safe), c => `%${`00${c.charCodeAt(0).toString(16)}`.slice(-2)}`)
      .join(''),
  );
};
const hasEmptySession = () => Object.getOwnPropertyNames(getSession()).length === 0;

const parseJwt = token => token && JSON.parse(b64DecodeUnicode(token.split('.')[1]));
...
const SessionManagerFactory = () => {
  const updateToken = token => setSession({ ...getSession(), token });
  const setToken = token => {
    ...
    session.context.user = { id: decoded.sub, name: decoded.userName, email: decoded.userEmail };
