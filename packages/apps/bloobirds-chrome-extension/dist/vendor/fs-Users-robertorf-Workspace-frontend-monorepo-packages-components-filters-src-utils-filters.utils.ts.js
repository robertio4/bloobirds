...
import { getBobjectFromLogicRole, isEmptyObject, isObject } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-utils-dist-index.js.js";
import spacetimeClass from "/vendor/.vite-deps-spacetime.js__v--14e7d295.js";
...
const getValueFromQuickFilter = (value) => {
  if (!value)
    return "";
  const { searchMode } = value;
  switch (searchMode) {
    case SearchMode.RANGE: {
      return Object.keys(value?.query)?.map((prop) => value?.query[prop]);
    }
...
const parseRelativeDates = (relativeDate) => {
  ...
  switch (relativeDate) {
    ...
    case RelativesDate.LAST_WEEK: {
      const lastTimeKey = relativeDate.split("_")[1];
      return {
    ...
    case RelativesDate.THIS_YEAR: {
      const thisTimeKey = relativeDate.split("_")[1];
      return {
...
export const parsedDateValueToRelativeDates = (dateValue) => {
...
const parseFilterRangeValue = (data) => {
  if (!data)
    return "";
  const { type } = data;
  switch (type) {
...
export const parseFilterRelativeDateValue = (data) => {
  if (!data)
    return "";
  return {
    query: {
...
const parseFilterRangeValueToFilter = (data) => {
  if (!data)
    return "";
  const { query } = data;
  if (!query)
...
const parseQuickFilterValue = (value) => {
  ...
  switch (searchType) {
    ...
    case SearchType.EXACT: {
      if (value?.bobjectPicklistValue === MatchRows.FULL)
        return MatchRows.FULL;
      if (value?.bobjectPicklistValue === MatchRows.EMPTY)
        return MatchRows.EMPTY;
...
const parseSubqueriesValues = (subqueriesValues) => Object.keys(subqueriesValues)?.reduce((parsedValues, filterKey) => {
...
const replaceConditionalFields = (filters) => {
  return Object.keys(filters)?.reduce((newFilters, filterId) => {
    const value = filters[filterId];
    newFilters = { ...newFilters, [filterId]: value };
    return newFilters;
  }, {});
};
export const removeFiltersById = (filters, filtersIds) => {
  return Object.keys(filters)?.reduce((newFilters, bobjectType) => {
    const filtersByBobjectType = filters[bobjectType];
    if (filtersByBobjectType && Object.keys(filtersByBobjectType).length) {
      Object.keys(filtersByBobjectType).forEach((fieldId) => {
        if (filtersIds.includes(fieldId)) {
          const filtersByBobjectType2 = { ...newFilters[bobjectType] };
          delete filtersByBobjectType2[fieldId];
          newFilters = {
            ...newFilters,
            [bobjectType]: {
              ...filtersByBobjectType2
            }
          };
        } else {
          newFilters = {
...
export const transformFiltersToFiltersState = (filterValue) => {
  let value;
  const isInputPickerValue = filterValue && isObject(filterValue) && (!!filterValue?.value || [MatchRows.EMPTY, MatchRows.FULL].includes(
    filterValue?.type
  ));
  ...
  if (Array.isArray(value) && value.includes(void 0)) {
    value = value.filter((item) => item !== void 0);
  }
  return value;
};
export const transformFilterStateToFilter = (filter) => {
  if (!filter)
    return void 0;
...
export const transformQuickFiltersToFilters = (filters, bobjectFields) => {
  return filters?.reduce((query, filter) => {
    ...
    const parsedValues = filterValue.map((value) => parseQuickFilterValue(value));
    return {
      ...query,
      [bobjectType]: {
        ...query[bobjectType],
        [filter.bobjectFieldId]: Array.isArray(parsedValues) && parsedValues?.length === 1 ? parsedValues[0] : parsedValues
      }
    };
  }, {});
};
export const transformMoreFiltersToFilters = (filters, bobjectFields) => {
  const fieldsIds = Object.keys(filters);
  return fieldsIds?.reduce((query, fieldId) => {
...
export const transformFiltersToMoreFilters = (filters) => {
  return Object.keys(filters).reduce((moreFilters, bobjectType) => {
    const isBobjectType = Object.values(BobjectTypes).includes(bobjectType);
    const filtersByBobjectType = filters[bobjectType];
    if (isBobjectType && typeof filtersByBobjectType === "object" && Object.keys(filtersByBobjectType).length) {
      Object.keys(filtersByBobjectType).forEach((fieldId) => {
        ...
        const isRelativeDateValue = fieldValue && Object.values(RelativesDate).includes(fieldValue?.type);
        ...
        if (isMatchValue) {
        ...
        } else if (Array.isArray(fieldValue) && [MatchRows.EMPTY, MatchRows.FULL].includes(fieldValue[0])) {
          filterValue = fieldValue?.map((value) => value);
        } else if (isRelativeDateValue) {
          const parsedDates = parseRelativeDates(fieldValue);
          filterValue = {
            query: Object.keys(parsedDates)?.reduce(
              (acc, key) => ({ ...acc, [key]: new Date(parsedDates[key]) }),
              {}
            ),
            searchMode: SearchMode.RANGE,
          ...
          filterValue = {
            ...
            searchType: fieldValue?.searchType ? fieldValue?.searchType : getTypeFromValue(fieldValue?.query),
            type: fieldValue?.type
          };
        }
...
export const transformFiltersToQuickFilters = (filters) => {
  if (!filters)
    return {};
  return Object.keys(filters).reduce((quickFilters, bobjectType) => {
...
export const transformFiltersToQuery = (filters, mainBobject, bobjectFields) => {
  return Object.keys(filters).reduce((query, bobjectType) => {
    ...
    const parsedFilters = Object.keys(bobjectTypeFilters).reduce(
      (parsedFilters2, fieldId) => {
        const fieldValue = bobjectTypeFilters[fieldId];
        const isRelativeDateValue = fieldValue && Object.values(RelativesDate).includes(fieldValue);
        const isRangeValue = fieldValue && isObject(fieldValue);
        const isDateRange = fieldValue && (fieldValue?.lte || fieldValue?.gt);
        if (!isRelativeDateValue && !isRangeValue) {
          return { ...parsedFilters2, [fieldId]: fieldValue };
          ...
          return { ...parsedFilters2, [fieldId]: value || fieldValue?.value };
        } else {
          const value = {
          ...
          return { ...parsedFilters2, [fieldId]: value };
        }
      },
      {}
    );
...
export const resetFiltersByBobjectType = (bobjectTypes = [], filters) => {
  const availableBobjectTypes = [BobjectTypes.Company, BobjectTypes.Lead, BobjectTypes.Opportunity];
  const includedBobjectTypes = availableBobjectTypes.filter(
    (bobjectType) => bobjectTypes.includes(bobjectType)
  );
  const bobjectFilters = {};
...
export const transformFilterBobjectTypeToORsState = (bobjectType) => {
  if (!Array.isArray(bobjectType)) {
    if (["Task", "Activity"].includes(bobjectType) || !bobjectType)
      return [];
    return [bobjectType];
  } else {
    return bobjectType.filter((type) => ["Task", "Activity"].includes(type));
  }
};
