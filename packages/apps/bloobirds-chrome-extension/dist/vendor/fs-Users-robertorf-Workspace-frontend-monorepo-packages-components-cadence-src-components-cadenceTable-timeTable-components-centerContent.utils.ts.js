...
export function getDateIndex(timeWindow, date) {
  let ret = date;
  switch (timeWindow) {
...
export function getCenterFromFirstDate(date) {
  const d = new Date(date);
  return format(new Date(d.setDate(d.getDate() + 20 - d.getDay() + 4)), "yyyy-MM-dd");
}
const getMinAndMaxDate = (tasks, currentDate) => {
...
function getColumns(timeWindow, minDate, maxDate) {
  ...
  switch (timeWindow) {
    ...
    case TIME_WINDOW.MONTHLY: {
      ...
      const ret = [];
      for (let date = new Date(new Date(minFirstOfMonth).setMonth(minFirstOfMonth.getMonth() - 2)); date < new Date(new Date(maxFirstOfMonth).setMonth(maxFirstOfMonth.getMonth() + 4)); date.setMonth(date.getMonth() + 1)) {
        ret.push(format(date, "yyyy-MM-dd"));
      }
      return ret;
    }
...
export function getDateList(timeWindow) {
