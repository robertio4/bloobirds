...
import { removeQueryParam } from "/src/utils/url.ts.js";
...
const injectFullName = (searchValue) => {
...
export const clickMessageButton = () => {
  isElementLoaded(messageLinkedinSelector).then(() => {
    const aButton = document.querySelector(messageLinkedinSelector);
  ...
  isElementLoaded(profileActionMessageLinkedinSelector).then(() => {
    const aButton = document.querySelector(messageLinkedinSelector);
    if (!aButton) {
      const secondaryMessageButton = document.querySelector(profileActionMessageLinkedinSelector);
    ...
    if (dropdown) {
      Array.from(dropdown.children).forEach((child) => {
        if (child.querySelector(sendMessageSvgSelector)) {
          child.querySelector(dropdownItemDivSelector)?.click();
        }
      });
      removeQueryParam("bb-messaging-tab-open");
    }
  ...
  isElementLoaded(moreLinkedInButton).then(() => {
    const aButton = document.querySelector(moreLinkedInButton);
    aButton?.click();
    removeQueryParam("bb-messaging-tab-open");
  });
  isElementLoaded(messageSalesNavSelector).then(() => {
    const aButton = document.querySelector(messageSalesNavSelector);
    aButton?.click();
    removeQueryParam("bb-messaging-tab-open");
  });
};
export function linkedinMessagesFromBBInjector() {
  ...
  if (window.location.pathname === "/messaging/thread/new/" && window.location.host === "www.linkedin.com") {
    const query = window.location.search.substring(1)?.split("&");
    const searchKey = query?.[0]?.split("=")?.[0];
