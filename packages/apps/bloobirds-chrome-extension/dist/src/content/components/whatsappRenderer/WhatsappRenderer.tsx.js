import { createHotContext as __vite__createHotContext } from "/vendor/vite-client.js";import.meta.hot = __vite__createHotContext("/src/content/components/whatsappRenderer/WhatsappRenderer.tsx.js");import RefreshRuntime from "/vendor/react-refresh.js";
let prevRefreshReg;
let prevRefreshSig;
if (import.meta.hot) {
  ...
  window.$RefreshReg$ = (type, id) => {
    RefreshRuntime.register(type, "/Users/robertorf/Workspace/frontend-monorepo/packages/apps/bloobirds-chrome-extension/src/content/components/whatsappRenderer/WhatsappRenderer.tsx " + id);
  };
  window.$RefreshSig$ = RefreshRuntime.createSignatureFunctionForTransform;
}
var _jsxFileName = "/Users/robertorf/Workspace/frontend-monorepo/packages/apps/bloobirds-chrome-extension/src/content/components/whatsappRenderer/WhatsappRenderer.tsx", _s = $RefreshSig$();
...
import { useDebouncedCallback, useIsB2CAccount } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-hooks-src-index.tsx.js";
import { PermissionType, BobjectTypes, ContactViewSubTab, MessagesEvents, LEAD_FIELDS_LOGIC_ROLE, MIXPANEL_EVENTS } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-types-src-index.ts.js";
import { api, getTextFromLogicRole, getExtensionBobjectByIdFields } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-utils-dist-index.js.js";
import __vite__cjsImport9_lodash_debounce from "/vendor/.vite-deps-lodash_debounce.js__v--ce8d7378.js"; const debounce = __vite__cjsImport9_lodash_debounce.__esModule ? __vite__cjsImport9_lodash_debounce.default : __vite__cjsImport9_lodash_debounce;
...
import { getValueFromLogicRole } from "/src/utils/bobjects.utils.ts.js";
import { useExtensionContext } from "/src/content/components/context.tsx.js";
import { WhatsappTemplateSelector } from "/src/content/components/templatesSelector/whatsappTemplateSelector.tsx.js";
import WhatsappButton from "/src/content/components/whatsappRenderer/components/WhatsappButton.tsx.js";
import { Status, getContactNumber, extractWhatsappMessageElements, extractContactName, processMessage } from "/src/content/components/whatsappRenderer/utils/whatsappRenderer.utils.tsx.js";
import { jsxDEV as _jsxDEV, Fragment as _Fragment } from "/vendor/id-__x00__react-jsx-dev-runtime.js";
const injectTemplateSelector = (leadId) => {
  const templateSelector = document.querySelector("[data-bb-template-selector]");
  if (templateSelector) {
    return;
  }
  const buttonsBox = document.querySelector("#main > footer > div.copyable-area > div > span:nth-child(2) > div > div");
  const newDiv = document.createElement("div");
  newDiv.setAttribute("data-bb-template-selector", "true");
  buttonsBox.appendChild(newDiv);
  ReactDOM.render(/* @__PURE__ */ _jsxDEV(WhatsappTemplateSelector, {
    leadId
  }, void 0, false, {
...
const WhatsappRenderer = () => {
  ...
  const enableAutoSync = autoSyncPermission === PermissionType.ENABLED || autoSyncPermission === PermissionType.FORCED;
  const {
    createToast
  } = useToasts();
  const {
  ...
  const updatePortal = (t2, portals2, newDiv, status, type = "message", propagate = true) => {
    const portal = portals2.current.find((portal2) => portal2.anchor === newDiv);
    if (!portal) {
      return;
    }
    portal.children = /* @__PURE__ */ _jsxDEV(WhatsappButton, {
      t: t2,
    ...
    if (propagate && type === "conversation") {
      portals2.current.filter((portal2) => portal2.source === "message").forEach((portal2) => {
        const message = messagesRef.current.find((message2) => message2.id === portal2.name);
        portal2.children = /* @__PURE__ */ _jsxDEV(WhatsappButton, {
          ...portal2.children.props,
          t: t2,
          type: "message",
          leadName: wsLead.current?.name,
  ...
  const handleOnClick = async (messages, newDiv, type = "message") => {
    if (wsLead.current && newDiv) {
      if (!messages)
        messages = messagesRef.current;
      try {
        updatePortal(t, portals, newDiv, Status.LOADING, type);
        const isCompany = wsLead.current?.id.includes("Company");
        await api.post("/messaging/whatsapp", messages.map((message) => {
          if (isCompany) {
            return {
              ...message,
              companyId: wsLead.current?.id,
              leadId: void 0
            };
          }
          return {
        ...
        window.dispatchEvent(new CustomEvent(MessagesEvents.SubTabUpdated, {
          detail: {
            subTab: ContactViewSubTab.ACTIVITIES
          }
        }));
      } catch (e) {
  ...
  const checkMessagesStatus = async () => {
    ...
    messagesRef.current = messagesRef.current.map((message) => {
      ...
      return {
        ...message,
        status: synced ? Status.SUCCESS : Status.ENABLED
      };
    });
  };
  const loadMessages = async () => {
    ...
    messagesRef.current.forEach((message) => {
      const portal = portals.current.find((portal2) => portal2.name === message.id);
      if (portal) {
        portal.children = /* @__PURE__ */ _jsxDEV(WhatsappButton, {
          t,
          type: "message",
        ...
        portals.current.push({
          ...
          name: message?.id,
          anchor: message?.div,
          children: /* @__PURE__ */ _jsxDEV(WhatsappButton, {
            t,
            type: "message",
            leadName: wsLead.current?.name,
            status: wsLead.current ? message.status : "disabled",
  ...
  const getLeadFromNumber = async (number, wsName) => {
    let wsOppId;
    let wsLeadId;
    let name;
    try {
      ...
      const includeTypes = isB2CAccount ? ["LEAD", "OPPORTUNITY"] : ["COMPANY", "LEAD", "OPPORTUNITY"];
      const response = await api.post("/calls/whiteLabel/search", {
        phoneNumber: "+" + number,
        includeTypes
      });
      if (response.status === 200) {
        let bobjects = response.data;
        if (bobjects.length === 1) {
          ...
          const opportunitiesIds = isLead && getValueFromLogicRole(bobject, LEAD_FIELDS_LOGIC_ROLE.OPPORTUNITIES);
          name = getTextFromLogicRole(bobject, type.toUpperCase() + (isLead ? "__FULL_NAME" : "__NAME"));
          if (opportunityAsMainBobject && opportunitiesIds) {
            if (!Array.isArray(opportunitiesIds)) {
              wsOppId = opportunitiesIds;
            } else {
              const result = await Promise.all(opportunitiesIds?.map((o) => getExtensionBobjectByIdFields({
                typeName: BobjectTypes.Opportunity,
                objectId: o?.split("/")[2]
              })) || []);
              wsOppId = null;
              bobjects = result.map((r) => r.data);
            }
          }
        }
        window.dispatchEvent(new CustomEvent(MessagesEvents.SubTabUpdated, {
          detail: {
            subTab: ContactViewSubTab.OVERVIEW
          }
        }));
      ...
      console.log(...oo_oo(`2816132507_343_6_343_20_4`, e));
    }
    if (!wsLeadId) {
      return null;
    }
  ...
  const insertButtonSyncConversation = (contactName, whatsappLead2) => {
    const header = document.querySelector("#main > header");
    if (!header) {
      return;
    }
    const isSingleChat = header.querySelector('[role="button"]:nth-child(2) > div > div')?.firstElementChild.tagName === "DIV";
    if (!isSingleChat) {
      return;
    }
    ...
    portals.current.push({
      ...
      children: /* @__PURE__ */ _jsxDEV(WhatsappButton, {
        t,
        type: "conversation",
        leadName: whatsappLead2?.name,
        status: whatsappLead2 ? "enabled" : "disabled",
        onClick: async () => {
  ...
  const debouncedExtractMessages = useDebouncedCallback(async () => {
    let contactLead;
    ...
    if (wsLead.current && nMessages !== messagesRef.current.length) {
      const divButton = document.querySelector("#main > header [data-bloobirds-button]");
      if (divButton) {
    ...
    setTimeout(() => {
      if (contactLead && messagesRef.current.length === 0) {
        document.querySelectorAll("[data-bloobirds-button]:not(#main > header [data-bloobirds-button])").forEach((element) => {
          element.remove();
        });
        document.querySelectorAll("[data-bb-processed]").forEach((element) => {
          element.removeAttribute("data-bb-processed");
    ...
    if (enableAutoSync && (contactName !== wsContactName.current || nMessages !== messagesRef.current.length)) {
      const conversationDiv = document.querySelector("#main > header [data-bloobirds-button]");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await loadMessages();
      updatePortal(t, portals, document.querySelector("#main > header [data-bloobirds-button]"), Status.ENABLED, "conversation", true);
      rerender((prev) => prev + 1);
    };
    if (whatsappLead) {
      fetchData();
...
_s(WhatsappRenderer, "l97a9w+HaeFEdMfT1zCZFttA+vs=", true, function() {
  return [useIsB2CAccount, useExtensionContext, useToasts, useExtensionContext, useTranslation, useDebouncedCallback];
});
_c = WhatsappRenderer;
...
oo_te;
var _c;
$RefreshReg$(_c, "WhatsappRenderer");
if (import.meta.hot) {
