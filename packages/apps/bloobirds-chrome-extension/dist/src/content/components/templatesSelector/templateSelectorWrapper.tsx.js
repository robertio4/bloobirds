import { createHotContext as __vite__createHotContext } from "/vendor/vite-client.js";import.meta.hot = __vite__createHotContext("/src/content/components/templatesSelector/templateSelectorWrapper.tsx.js");import RefreshRuntime from "/vendor/react-refresh.js";
let prevRefreshReg;
let prevRefreshSig;
if (import.meta.hot) {
  ...
  window.$RefreshReg$ = (type, id) => {
    RefreshRuntime.register(type, "/Users/robertorf/Workspace/frontend-monorepo/packages/apps/bloobirds-chrome-extension/src/content/components/templatesSelector/templateSelectorWrapper.tsx " + id);
  };
  window.$RefreshSig$ = RefreshRuntime.createSignatureFunctionForTransform;
}
var _jsxFileName = "/Users/robertorf/Workspace/frontend-monorepo/packages/apps/bloobirds-chrome-extension/src/content/components/templatesSelector/templateSelectorWrapper.tsx", _s = $RefreshSig$(), _s2 = $RefreshSig$(), _s3 = $RefreshSig$();
...
import { Dropdown, Icon, Spinner, Text, Tooltip, useVisible } from "/vendor/.vite-deps-@bloobirds-it_flamingo-ui.js__v--2cc8757f.js";
import { useActiveUserSettings, useDataModel } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-hooks-src-index.tsx.js";
...
import { Environment } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-types-src-index.ts.js";
import { replaceWithContent, serializeMessagingTemplate } from "/vendor/fs-Users-robertorf-Workspace-frontend-monorepo-packages-utils-dist-index.js.js";
import { TemplateSelectorPlaces } from "/src/types/messagingTemplates.ts.js";
import { fetchLead, scrapAndSync } from "/src/utils/scrapper/messages/templateSelector/metaScrapper.tsx.js";
import { normalizeUrl, removeQueryParams } from "/src/utils/url.ts.js";
...
function ResyncMessages({
  ...
  const {
    ...
    keyPrefix: "templateSelector.resync"
  });
  async function scrap() {
    if (!scrapping && lead) {
      setScrapping(true);
      await scrapAndSync(normalizeUrl(window.location.href), bubbleEl, place, lead.id.value, leadSetRef);
      setScrapping(false);
    }
  }
  return /* @__PURE__ */ _jsxDEV("div", {
    className: styles.resyncButtonContainer,
    id: "resync-bb-msg-btn",
    children: /* @__PURE__ */ _jsxDEV(Tooltip, {
      title: t("tooltip"),
      position: "top",
      children: /* @__PURE__ */ _jsxDEV("div", {
        className: styles.resyncButton,
        onClick: () => !scrapping && scrap(),
        children: scrapping ? /* @__PURE__ */ _jsxDEV(Spinner, {
          name: "loadingCircle",
          color: "bloobirds",
          size: 16
        }, void 0, false, {
...
_s(ResyncMessages, "ZHtT60CX2V47DyNzS7KKBZDYiYc=", false, function() {
  return [useTranslation];
});
_c = ResyncMessages;
const getElement = (place, bubbleContext) => {
  if (place === TemplateSelectorPlaces.LinkedinChat) {
    return bubbleContext.querySelector(".msg-s-message-list-container ul.msg-s-message-list-content");
  } else if (place === TemplateSelectorPlaces.Linkedin) {
    return document.querySelector("main div.msg-s-message-list-container ul.msg-s-message-list-content");
  } else if (place === TemplateSelectorPlaces.SalesNavigator) {
    return document.querySelector(".thread-container ul");
  } else if (place === TemplateSelectorPlaces.SalesNavigatorChat) {
    return document.querySelector(".message-overlay__conversation ul");
  }
  return null;
};
function useFetchLead(lead, place, bubbleContext, setLead, setLoading, currentPage, leadSetRef) {
  _s2();
  const currentTemplateRef = useRef(true);
  const observerRef = useRef(null);
  const debounceTimeoutRef = useRef(null);
  useEffect(() => {
    if (currentTemplateRef.current && currentPage === normalizeUrl(window.location.href)) {
      if (lead === null) {
        leadSetRef.current = false;
        setLoading(true);
        fetchLead(place, bubbleContext, setLead, setLoading, currentPage, currentTemplateRef).catch((er) => console.log(...oo_oo(`3699105526_94_23_94_38_4`, er))).then(() => leadSetRef.current = true);
      } else {
        if (!observerRef.current) {
          const el = getElement(place, bubbleContext);
          if (!el) {
            return;
          }
          observerRef.current = new MutationObserver((mutationsList) => {
            ...
            debounceTimeoutRef.current = setTimeout(() => {
              for (const mutation of mutationsList) {
                if (mutation.type === "childList" && mutation.addedNodes.length > 0 && mutation.addedNodes[0]?.className?.includes("last-msg") && lead) {
                  scrapAndSync(currentPage, bubbleContext, place, lead.id.value, currentTemplateRef);
                }
              }
            }, 1e3);
          });
...
_s2(useFetchLead, "LfxpbIcWqPajYWtLom6neV2zNjw=");
export const TemplateSelectorWrapper = ({
  ...
  const addTemplateToTextArea = (content) => {
    if (place === TemplateSelectorPlaces.SalesNavigator || place === TemplateSelectorPlaces.SalesNavigatorChat) {
      const textArea = parentForm.querySelector(".compose-form__message-field");
      if (textArea) {
        textArea.value = content.replace(/<(?:br|\/div|\/p)>/g, "\n").replace(/<.*?>/g, "");
        textArea.focus();
        setVisible(false);
      }
    } else if (place === TemplateSelectorPlaces.Linkedin || place === TemplateSelectorPlaces.LinkedinChat) {
      const div = parentForm.querySelector(".msg-form__contenteditable");
      const placeHolderDiv = parentForm.querySelector('div[class^="msg-form__placeholder"]');
      if (div) {
  ...
  const handleAdd = ({
    ...
    if (lead?.id || leadIdValue) {
      serializeMessagingTemplate(id, leadIdValue || lead?.id?.objectId, companyIdValue || lead?.companyId).then((data) => {
        const content = data?.data?.serializedTemplate;
        addTemplateToTextArea(replaceWithContent(content, lead?.nameTo, lead?.jobTitle, lead?.companyName, userData?.user?.name));
      }).catch(() => {
        addTemplateToTextArea(replaceWithContent(fallbackContent, lead?.nameTo, lead?.jobTitle, lead?.companyName, userData?.user?.name));
      });
      removeQueryParams();
    } else {
      addTemplateToTextArea(replaceWithContent(fallbackContent, lead?.nameTo, lead?.jobTitle, lead?.companyName, userData?.user?.name));
  ...
  return /* @__PURE__ */ _jsxDEV("div", {
    children: [/* @__PURE__ */ _jsxDEV("div", {
      ...
      children: [lead?.id?.value && /* @__PURE__ */ _jsxDEV(ResyncMessages, {
        bubbleEl,
        place,
        lead,
        leadSetRef
        ...
        anchor: /* @__PURE__ */ _jsxDEV("div", {
          ...
          children: [loading ? /* @__PURE__ */ _jsxDEV(Spinner, {
            ...
            children: lead && lead?.id ? /* @__PURE__ */ _jsxDEV(Tooltip, {
          ...
          }, void 0, false), /* @__PURE__ */ _jsxDEV(BloobirdsLogo, {
            width: 20,
            height: 20,
            fill: "#1991FF"
          }, void 0, false, {
...
_s3(TemplateSelectorWrapper, "DaE4/T9tVvetiU+NIm4wsHATOHQ=", false, function() {
  return [useVisible, useTranslation, useActiveUserSettings, useDataModel, useFetchLead];
});
_c2 = TemplateSelectorWrapper;
...
oo_te;
var _c, _c2;
$RefreshReg$(_c, "ResyncMessages");
$RefreshReg$(_c2, "TemplateSelectorWrapper");
if (import.meta.hot) {
