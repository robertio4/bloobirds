...
function getBobjectIsSalesStage(stageId, bobjectType, dataModel) {
  switch (bobjectType) {
    case "Opportunity":
      return true;
    case "Company":
...
function getBobjectStatus(bobject, bobjectType, isSalesStage, dataModel) {
  if (bobjectType === "Opportunity")
    return dataModel.findValueById(bobject.status);
...
function getSalesforceStatus(bobject, dataModel) {
  ...
  const salesStatusValue = bobject?.salesforceStatus ?? bobject?.salesforceStage ?? bobject?.rawBobject?.[salesforceStatusField?.id] ?? getValueFromLogicRole(bobject, salesforceStatusLogicRole);
  return salesforceStatusField?.values?.find(
    (value) => (value?.salesforceLabel === salesStatusValue || value?.name === salesStatusValue || value?.id === salesStatusValue) && (bobject?.id?.typeName != BobjectTypes.Lead || value.salesforceObjectType == sobjectType)
  );
}
