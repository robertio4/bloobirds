...
it('should reach "Configure cadence" from "Next steps" when the event "NEXT" occurs after transitioning to "Configure cadence" with a Configure a new cadence', () => {
  const expectedValue = STEPS.CADENCE_FEEDBACK;
...
it('should reach "Categories feedback" from "Configure cadence" when the event "NEXT" occurs after transitioning to "Categories Feedback"', () => {
  const expectedValue = STEPS.CADENCE_FEEDBACK;
...
it('should reach "Update lead status" from "Categories feedback" when the event "NEXT" occurs after transitioning to "Update lead status" when there are leads', () => {
  const expectedValue = STEPS.UPDATE_LEADS_STATUSES;
  const firstState = stepsMachine.transition(STEPS.CADENCE_FEEDBACK, {
    type: EVENTS.NEXT,
    hasLeads: true,
  });
  const actualState = stepsMachine.transition(firstState, EVENTS.NEXT);
