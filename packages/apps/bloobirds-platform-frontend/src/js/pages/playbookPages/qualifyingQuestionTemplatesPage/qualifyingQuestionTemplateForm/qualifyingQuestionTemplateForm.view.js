...
import { Button, IconButton, Input, Item, Select, Text, Tooltip } from '@bloobirds-it/flamingo-ui';
...
const QualifyingQuestionTemplateFormView = ({ existingQQ, mode, onCancel, onSave }) => {
  ...
  const defaultValues = {
    ...
    stage:
      existingQQ?.stage || isNoStatusPlanAccount ? TemplateStage.Sales : TemplateStage.Prospecting,
    segmentationValues: existingQQ?.segmentationValues,
    isLayoutReadOnly: mode === 'CREATION' ? false : !existingQQ?.isLayoutReadOnly,
  };
  ...
  const onSubmit = async data => {
    ...
    const templatePayload = {
      ...
      isLayoutReadOnly: !data?.isLayoutReadOnly,
    };
    if (mode === 'CLONE') {
      await onSave({
        ...templatePayload,
  ...
  const handleRemove = (answer, index) => {
    if (isAutogeneratedId(answer.id)) {
          ...
          <div className={messagingFormStyles._editor}>
            ...
            <main>
              {(methods.watch('type') === QQ_TYPES.GLOBAL_PICKLIST ||
                methods.watch('type') === QQ_TYPES.MULTI_GLOBAL_PICKLIST) && (
                <section>
                  ...
                  <Text size="s" color="softPeanut">
                    ...
                    <a href="https://support.bloobirds.com/hc/en-us/articles/360021529220-Create-and-edit-Qualifying-questions">
                      Knowledge Base
                    </a>
                    .
                  </Text>
